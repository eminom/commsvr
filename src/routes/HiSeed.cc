...
void get_seed(http_request* request, hw_http_response* response, void* user_data) {
	...
	char buff[BUFSIZ] = "";
	snprintf(buff, sizeof(buff), "0x%08x", _XxhashSeed);
    SetCString(body, buff);
    ...
    hw_http_response_send(response, (void*)"get_seed", response_complete);
}
